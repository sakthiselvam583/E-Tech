...
//import "../../screens/CMSUser/style.css";
import "../../E-Tech/videoStyle.css";
...
class NewVideoPlayer extends Component {
  ...
  async componentDidMount() {
    ...
    this.setState({
      ...
      videoUrl:this.props.Video.media
    });
...
console.log('testset')
    ...
    // JSON.parse(result.overlay).map(async (ival)=>{
    //   console.log(ival.quizId)
     

    //    if(ival.overlayType=="Quiz")
    //    {
    //     console.log(ival.overlayType)
    //     console.log(` id =${ival.quizId} and companyid=${JSON.parse(localStorage.getItem("Userdata")).customerId} `)

    //     let quizList = await CmsContent.getFreedom(
    //       " tbl_question.*",
    //       " tbl_question",
    //       ` id =${ival.quizId} and customerid=${JSON.parse(localStorage.getItem("Userdata")).customerId} `,
    //       "id",
    //       "id DESC"
    //     )
   ...
   // let pageName = result.data[0].name;
    ...
    // this.setState({ pageName });
      ...
      overlayTimeSetArray = [];
      if (this.props.Video.overlay) {

        videoOverlay = JSON.parse(this.props.Video.overlay);
          ...
          JSON.parse(result.overlay).map(async (ival)=>{
           // console.log(ival.quizId)
             ...
             // console.log(ival.overlayType)
              ...
              result.quizList=quizList.data[0];
              JSON.parse(result.quizList.contentJson).map((list, index) => {
              //  console.log(list)
                quizJson.push([list]);
                let correctJson = {};
  ...
  videoinsert = async(insertcheckvalue)=>{
    ...
    if(this.state.insertcheckvalue==1)
    {
     ...
     var timesDate =  year +"-" +month + "-" +day +" " + hour +":" + minutes +":" +seconds+'';
      ...
      userlog.append('userid',JSON.parse(localStorage.getItem("Userdata")).id)
      ...
      userlog.append('customerId',JSON.parse(localStorage.getItem("Userdata")).customerId)
          ...
          if (startindex === index) {
            ...
            if (quizList.length - 1 === index) {
              ...
              quizQuestionAppend.push(
                <button
                  type="button"
                  className="btn btn-success btn-sm btn-radius btnwidth resbutton"
      ...
      overlayListArray.map((overlay, index) => {
        if (
                  ...
                  <div className="col-sm-12">
                    ...
                    overlay.overlayType === "Load Url" ? (
                      <a
                        ...
                        target="_blank"
                        class="btn btn-success btn-sm btn-radius btnwidthNew resbutton"
                      >
                        OK
                      </a>
                      ...
                      <button
                        type="button"
                        className="btn btn-success btn-sm btn-radius btn-newWidth resbutton"
          ...
          if (viewBanner && overlay.overlayType === "Ad Banner") {
            overlayAppend = (
              <div
                className="card-body body-overflow hidescroll"
        ...
        {videoUrl && (
          <figure id="videoContainer" data-fullscreen="false">
            <video
              ...
              src={ACCESS_POINT+'/superAdmin/file?fileurl='+this.state.videoUrl}
            >
              {this.props.playfrom && (
