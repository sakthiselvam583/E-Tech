...
class StudentHeader extends Component { 
  ...
  async componentDidMount() {
    ...
    let  selectGrade = await CmsContent.getFreedom('tbl_grade_master.*','tbl_grade_master',`customerId =199 and id= ${Userdata.grade}`,'id','id'); 
 
    let  maptogroupuser = await CmsContent.getFreedom('tbl_maptogroup.*','tbl_maptogroup',`customerId =${Userdata.customerId} and userTypeId=10  and  groupid= ${Userdata.grade}`,'id','id');  
...
console.log(result)
  ...
  if(ival.name=='Assessments')
  {

    content.push(<li>
        <Link to={'StuViewAssessments'} >
         {ival.name} 
          </Link>
        </li>)
  }
  else if(ival.name=="Classwork")
{
  content.push(<li>
    <Link to={'StuViewClasswork'} >
     {ival.name} 
      </Link>
    </li>)

}
else if(ival.name=="Homework")
{

  content.push(<li>
    <Link to={'StuViewHomework'} >
     {ival.name} 
      </Link>
    </li>)

}
else if(ival.name=="Tutorial")
{

  content.push(<li>
    <Link to={'StuViewTutorial'} >
     {ival.name} 
      </Link>
    </li>)
