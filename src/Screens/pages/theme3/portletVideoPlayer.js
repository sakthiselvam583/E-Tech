...
class PortletVideoPlayer extends Component {
	constructor(props) {
    super(props);
    this.state = {
      pageList: "",
  ...
  /*  let userlog = localStorage.getItem("userDetails");
	  userlog = JSON.parse(userlog);
	   ...
	   //let userlog =[{id:433,userName:'karan'}];
  ...
  await this.getContent(this.props.data);
   // this.setState({dataDD:this.props.data}); 
  }
 
async getContent(data) {
	 const { correctAnswer } = this.state;
	 this.setState({insertcheckvalue:1}); 
	 let quizJson = [];
	...
	if (data.overlay) {
		...
		videoOverlay.map(async (jsonContent, index) => {
			...
			this.setState({
			  ...
			  let userId = this.state.userId ? this.state.userId : 433 ;
	...
	let checkResume = await localStorage.getItem(
	 ...
	 this.setState({
        pageList: data,
        videoUrl: `${data.url}#t=${checkResume}`,
	 ...
	 /* let checkSurvey=JSON.parse(data.overlay);
	     if(checkSurvey[0].surveyUrl){
			 this.Survey();
		 }*/
	}
	setInterval(() => {
        ...
        let userId = this.state.userId ? this.state.userId : null ;
 ...
 xButton=()=>{
  ...
  Survey=async()=>{
	 /* try{
	  let surveyId=this.state.overlayListArray[0].surveyUrl;
	  let SurveyList = await CmsContent.getFreedom(
        "*",
        "tbl_survey,tbl_surveyTitle",
        `tbl_survey.id=${surveyId} and tbl_surveyTitle.surveyId =${surveyId}`,
        1, 
        "tbl_survey.id" 
      );
	 ...
	 // console.log(SurveyQuestion); 
      let mappedSurvey = await CmsContent.getFreedom(
        "*",
        "tbl_survey_mapping",
        `tbl_survey_mapping.surveyid =${surveyId}`,
        1,
        "id"
      );
	...
	let Answer = [];
	 ...
	 window.open(`https://difuza.com/DF/SurveyForm/${this.state.userId}/${this.state.overlayListArray[0].surveyUrl}/${Admin.id}`,"_blank");
              ...
              <div className="centered">
				...
				<div className="row form-group ">
				  ...
				  {overlay.overlayType === "Load Url"&&(
				     <Iframe url={`https://${overlay.loadUrl}`}    
                          width="450px"
                          height="250px" 
						  id="myId" 
      ...
      <div>
        {videoUrl && (
          <figure id="videoContainer" data-fullscreen="false">
            ...
            <div id="banner"  className="Mobile" style={{ display: "none", background: "red" }}>
              {overlayAppend}
            </div>
...
export default PortletVideoPlayer;